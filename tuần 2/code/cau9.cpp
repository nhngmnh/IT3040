...
void precalc() {
    ...
    {
        sigmoid[i]=1.0/(1.0+exp(-tmp)); tmp+=delta;
    }
    /*****************/
}
...
inline double sigmoid_fast(double x) {
    /*****************/
    if (x>limtren) return 0;
    else if (x<limduoi) return 1;
    else
    {   int i=floor((x-limduoi)/delta);
        return sigmoid[i]+(sigmoid[i+1]-sigmoid[i])*(x-limduoi-delta*i)/delta;
    }
    /*****************/
}
//# END fast code
